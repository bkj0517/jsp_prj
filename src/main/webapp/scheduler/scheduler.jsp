...
<style type="text/css">
 ...
 .calTab > thead > tr > th {width:100px; height: 30px ; border:1px solid #CECECE; text-align: center}
 .calTab > tbody > tr > td {width:100px; height: 80px ; border:1px solid #CECECE; vertical-align: top; padding-left: 5px; font-weight: bold}
...
<%
   ...
   //입력되는 년, 월을 사용하여 다른 날짜로 설정
   if(paramYear != null){//이게 null이 아니라는 건, 사용자가 < 혹은 >를 클릭했다는거지.
      cal.set(Calendar.DAY_OF_MONTH,1);//모든 월에 있는 날짜로 설정되어야 날짜 이동
      cal.set(Calendar.YEAR, Integer.parseInt(paramYear));
      cal.set(Calendar.MONTH, Integer.parseInt(paramMonth)-1);//0~11월 이므로 -1을 해준다.
   }//end if
...
<%--    <a href="scheduler.jsp?year=<%= nowMonth-1==0?nowYear-1:nowYear %>&month=<%=nowMonth-1==0?12:nowMonth-1 %>">
      <span title="이전 월" class="prevMonth" >
      &lt;
      </span>
   </a>
   ...
   <a href="javascript:moveCalendar('p',<%= nowYear%>,<%=nowMonth-1 %>)">
      <span title="이전 월" class="prevMonth" >
      &lt;
      </span>
   </a>
   <span class="mainDate"><%= nowYear %>.<%= nowMonth %></span>
   <a href="javascript:moveCalendar('n',<%=nowYear%>,<%=nowMonth+1%>)">
   <span title="다음 월" class="nextMonth">&gt;</span>
   </a>
   <a href="javascript:moveCalendar('n',<%=flagYear%>,<%=flagMonth%>)"><span title="오늘" class="btnToDay">오늘</span></a>
   </div>
