...
<style type="text/css">
 ...
 #calenderWrap{ width:860px;}
 #calenderHeader{ text-align: center; font-size: 25px; font-weight: bold; 
 ...
 .calTab > thead > tr > th { width:100px; height: 30px; border: 1px solid #CECECE;
  text-align: center;}
 .calTab > tbody > tr > td { width:100px; height: 80px; border: 1px solid #CECECE; 
	 vertical-align: top; padding-left: 5px; font-weight: bold; }

.sunTitle{ background-color: #FF6C6C; color:#FFFFFF}
/* weekTitle{} */
.satTitle{ background-color: #ff6c6c; color: #ffffff}
/* 요일별 색 */

 .sunTitle{ background-color: #FF6C6C; }
 .weekTitle{}
 .satTitle{background-color: #6CC0FF;}
...
<%
	...
	boolean flagToday=flagDate.toString().equals(paramYear + paramMonth);
...
<a href="scheduler.jsp?year=<%=nowMonth+1==13? nowYear+1:nowYear %>&month=<%= nowMonth+1==13? 1:nowMonth+1%>"><span title="다음 월" class="nextMonth">&gt;</span></a>
	...
	<a href="scheduler.jsp">오늘</a></span>
</div>
<div id="calenderContainer">
	...
	<% //1일의 요일 출력 - 1일을 출력하기 전 공백 설정
		...
		for(int tempDay=1; ;tempDay++){
			...
			//( Calender 특성 : 현재 월에 없는 날짜가 들어가면 자동으로 다음 달 1일로 설정. )
			...
			if( cal.get(Calendar.DAY_OF_MONTH) != tempDay) {
				/* log("----"+(cal.get(Calendar.DAY_OF_MONTH)+1)+
						","+cal.get(Calendar.DAY_OF_MONTH)); */
				//마지막 날이면 <td>를 만들고, 반복문을 빠져 나간다.	
			...
			<% 
			//토요일이면 줄을 변경 (Calender.DAY_OF_WEEK )
			switch(cal.get(Calendar.DAY_OF_WEEK )){
