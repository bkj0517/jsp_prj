...
<% 	int sum=0;
 	for(int i=1;i<101;i++){
 		 sum+=i;
 	}//end for - <%를 사용하면 한줄이 띄어진다.
%>
<div>
1~100까지의 합 은 <span><%=sum %></span>입니다.
</div>
<div>
1~100까지의 합 은 <span class="span"><%=sum %></span>입니다.
</div>
<br>
...
<div class="card" style="width: 18rem; float: left; margin-right : 10px">
  <img style="width : 280px; height : 160px" src="http://192.168.10.84/jsp_prj/common/images/<%=dto.getImg() %>" class="card-img-top" alt="...">
  <div class="card-body">
    <h5 class="card-title"><%=dto.getTitle() %></h5>
    <p class="card-text"><%=dto.getInfo() %></p>
    <a href="<%=dto.getUrl()%>" class="btn btn-primary">링크</a>
  </div>
</div>
<%}//end for %>
</div>
