...
public class BoardDAO {
	...
	public List<BoardDTO> selectBoard(RangeDTO rDTO) throws SQLException {
		...
		try {
			...
			selectBoard.append("	select num, subject, id, input_date, cnt	")
	...
	public BoardDTO selectOneBoard(int num) throws SQLException {
		...
		try {
			...
			selectOneBoard.append("	select subject,content,id,input_date,ip,cnt		")
			...
			if (rs.next()) {
				...
				// bDTO.setContent(rs.getString ("content"));
				// CLOB는 긴 문자열을 저장하므로 별도에 스트림을 연결하여 값을 읽어들인다.
